...
public class StepDefinitions extends BaseTest {
	...
	@Given("Login to the user with {string} and {string}")
	public void login_to_the_user_with_and(String string, String string2) {
		plp=loginpage.loginApplication(string, string2);
	}
	@When("click on the product with name {string}")
	public void click_on_the_product_with_name(String string) {
		cartpage=plp.addProductToCart(string);
	}
	@When("Click on the minicart button")
	public void click_on_the_minicart_button() {
		cartpage.clickCartButton();	
	}
	@When("Compare product in cart with given product name {string}")
	public void compare_product_in_cart_with_given_product_name(String string) {
		Assert.assertTrue(cartpage.comparingProdsToGetRequiredProd(string));
	}
	...
	@Then("check the confirm message is matching with {string}")
	public void check_the_confirm_message_is_matching_with_thankyou_for_the_order() {
